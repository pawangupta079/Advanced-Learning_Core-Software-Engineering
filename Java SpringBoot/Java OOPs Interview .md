...
A: A static method in an interface is defined with implementation and can be called using the interface name, not overridden by implementing classes.
...
A: A marker interface has no methods or fields; it is used to signal metadata to the JVM/compiler. Example: Serializable, Cloneable.



Q87. What is a functional interface?
A: A functional interface has exactly one abstract method and can be used with lambda expressions. Example: Runnable, Comparator.
...
A: Yes, it can have multiple default/static methods, but only one abstract method.
